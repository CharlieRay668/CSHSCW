    ...
    if response.method == "POST":
        ...
        if form.is_valid():
            ...
            if user is not None:
                ...
                message = "Hello {0},\n Recently someone requested to change your password for CSHS Website. If that was not you ignore this email. Click the link below to change you password.\n {1}".format(user.username, activation_link)
...
def register(response):
    if response.method == "POST":
        ...
        if form.is_valid():
            ...
            # message = "Hello {0}, Please use the link below to activate your account.\n {1}".format(user.username, activation_link)
            # to_email = form.cleaned_data.get('email')
            # email = EmailMessage(mail_subject, message, to=[to_email])
            # email.send()
